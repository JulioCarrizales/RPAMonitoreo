...
def descargar_archivo(lote_numero):
    ...
    nuevo_nombre = os.path.join(ruta_destino, f"truesight_noretiro_lote_{lote_numero}_{int(time.time())}.csv")
    try:
        os.rename(ruta_descarga, nuevo_nombre)
        print(f"Archivo del lote {lote_numero} descargado y movido a {nuevo_nombre}")
    ...
    if total_events > 0:
        ...
        for lote_numero in range(1, lotes + 1):
            ...
            if eventos_restantes > 0:
                # Elimina las filas ya procesadas (por ejemplo, aplicando un filtro o refrescando la página)
                # Aquí puedes agregar lógica para actualizar la interfaz y asegurarte de no procesar las mismas filas
                # Por simplicidad, asumiremos que refrescar la página actualiza la lista
